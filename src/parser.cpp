...
{
    // Lex the input.
    auto token_list = jsonlang_lex(file, input);
...
AST *jsonlang_parse(Allocator *alloc, const std::string &file, const char *input)
{
    // Parse the actual file.
    AST *expr = do_parse(alloc, file, input);

    // Now, implement the std library by wrapping in a local construct.
    auto *std_obj = static_cast<Object*>(do_parse(alloc, "std.jsonlang", STD_CODE));
...
static std::string unparse(const AST *ast_)
{
    ...
    if (auto *ast = dynamic_cast<const Apply*>(ast_)) {
        ...
        ss << "importstr " << jsonlang_unparse_escape(ast->file);
        ...
        ss << jsonlang_unparse_escape(ast->value);
...
std::string jsonlang_unparse_jsonlang(const AST *ast)
{
