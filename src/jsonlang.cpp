...
static char *import_callback (void *ctx_, const char *dir, const char *file, int *success)
{
    ...
    while (status == IMPORT_STATUS_FILE_NOT_FOUND) {
        if (jpaths.size() == 0) {
            *success = 0;
            const char *err = "No match locally or in the Jsonlang library path.";
            char *r = jsonlang_realloc(ctx.vm, nullptr, std::strlen(err) + 1);
            std::strcpy(r, err);
            return r;
        }
    ...
    if (status == IMPORT_STATUS_IO_ERROR) {
        *success = 0;
        char *r = jsonlang_realloc(ctx.vm, nullptr, err_msg.length() + 1);
        std::strcpy(r, err_msg.c_str());
        return r;
    } else {
        assert(status == IMPORT_STATUS_OK);
        *success = 1;
        char *r = jsonlang_realloc(ctx.vm, nullptr, input.length() + 1);
...
void usage(std::ostream &o)
{
    ...
    o << "Note that since jsonlang programs can begin with -, it is advised to\n";
    o << "use -- with -e if the program is unknown, e.g. jsonlang -e -- \"$CODE\".";
    o << std::endl;
}
    ...
    try {
        ...
        for (unsigned i=0 ; i<args.size() ; ++i) {
            ...
            if (arg == "-h" || arg == "--help") {
                ...
                jsonlang_ext_var(vm, var.c_str(), val);
        ...
        jsonlang_import_callback(vm, import_callback, &import_callback_ctx);

        int error;
        char *output;
        if (multi) {
            output = jsonlang_evaluate_snippet_multi(vm, filename.c_str(), input.c_str(), &error);
        } else {
            output = jsonlang_evaluate_snippet(vm, filename.c_str(), input.c_str(), &error);
        }
